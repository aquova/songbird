...
impl Cpu {
    ...
    pub fn ld_n_n(&mut self, target_reg: Regs, source_reg: Regs) {
        let source = self.get_reg(source_reg);
        self.set_reg(target_reg, source);
    }
    ...
    pub fn add_a_n(&mut self, reg: Regs, adc: bool) {
        let carry = 0;
        if adc && self.get_flag(Flags::C) {
            let carry = 1;
        }

        let sum = (self.get_reg(Regs::A) as u16) + (self.get_reg(reg) as u16) + carry;
        self.clear_flag(Flags::N);
        if sum > 0xFF {
            self.set_flag(Flags::C);
            self.set_flag(Flags::H);
    ...
    pub fn add_nn_nn(&mut self, high_target: Regs, low_target: Regs, high_source: Regs, low_source: Regs) {
        self.clear_flag(Flags::N);

        let upper_sum = (self.get_reg(high_target) as u16) + (self.get_reg(high_source) as u16);
        let lower_sum = (self.get_reg(low_target) as u16) + (self.get_reg(low_source) as u16);
    ...
    pub fn add_nn_16(&mut self, high_target: Regs, low_target: Regs, high_val: u8, low_val: u8) {
        self.clear_flag(Flags::N);
    ...
    pub fn sub_a_n(&mut self, reg: Regs, sbc: bool) {
        let carry = 0;
        if sbc && self.get_flag(Flags::C) {
            let carry = 1;
        }

        let diff: i16 = (self.get_reg(Regs::A) as i16) - (self.get_reg(reg) as i16) - carry;
        self.set_flag(Flags::N);
    ...
    pub fn and_a_n(&mut self, reg: Regs) {
        let mut A = self.get_reg(Regs::A);
        A &= self.get_reg(reg);
        self.clear_flag(Flags::N);
        self.set_flag(Flags::H);
    ...
    pub fn or_a_n(&mut self, reg: Regs) {
        let mut A = self.get_reg(Regs::A);
        A |= self.get_reg(reg);
        self.clear_flag(Flags::N);
        self.clear_flag(Flags::H);
    ...
    pub fn xor_a_n(&mut self, reg: Regs) {
        let mut A = self.get_reg(Regs::A);
        A ^= self.get_reg(reg);
        self.clear_flag(Flags::N);
        self.clear_flag(Flags::H);
    ...
    pub fn cp_a_n(&mut self, reg: Regs) {
        let diff = (self.get_reg(Regs::A) as i16) - (self.get_reg(reg) as i16);
        self.set_flag(Flags::N);
        if diff == 0 {
            self.set_flag(Flags::Z);
        } else {
